...
uint256 hashGenesisBlock("0x4e56204bb7b8ac06f860ff1c845f03f984303b5b97eb7b42868f714611aed94b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Einsteinium: starting difficulty is 1 / 2^12
...
{
    // Einsteinium: IsDust() detection disabled, allows any valid dust to be relayed.
...
// For Einsteinium also add the protocol rule that the first output in the coinbase must go to the charity address and have at least 2.5% of the subsidy (as per integer arithmetic)
	...
	int64 charityAmount = GetBlockValue(pindex->nHeight, 0) * 2.5 / 100;
...
bool SetBestChain(CValidationState &state, CBlockIndex* pindexNew)
{
    ...
    printf("SetBestChain: new best=%s  height=%d  log2_work=%.8g  tx=%lu  date=%s progress=%f\n timestamp=%lld",
      ...
      DateTimeStrFormat("%Y-%m-%d %H:%M:%S", pindexBest->GetBlockTime()).c_str(),
      Checkpoints::GuessVerificationProgress(pindexBest),pindexBest->GetBlockTime());
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Einsteinium: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Einsteinium: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6259347489c26417e91aa6690859fb2ed66a899d24598924a24927162edcd73f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        assert(block.hashMerkleRoot == uint256("0xb3e47e8776012ee4352acf603e6b9df005445dcba85c606697f422be3cc26f9b"));
...
unsigned char pchMessageStart[4] = { 0xe8, 0xf1, 0xc4, 0xac }; // Einsteinium: increase each by adding randoim value to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EinsteiniumMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EinsteiniumMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EinsteiniumMiner(CWallet *pwallet)
{
    printf("EinsteiniumMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("einsteinium-miner");
    ...
    try { loop {
        ...
        printf("Running EinsteiniumMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EinsteiniumMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EinsteiniumMiner, pwallet));
}
